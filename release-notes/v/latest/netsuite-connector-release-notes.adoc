...
* *Support for SuiteTalk API version 2017.1 GA* - No new web services. For a detailed list of changes, log into NetSuite, open *Help Center* and navigate to *New Release* -> *Release Notes* -> *NetSuite 2017.1 Release Notes* -> *SuiteTalk (Web Services) Integration*.
...
* *add-record with recordType=ITEM_FULFILLMENT*. In version 7.5.1 when trying to use add-record with recordType=ITEM_FULFILLMENT and item fulfillment record included _package_, the connector throwed a _java.lang.NoSuchFieldException: package_. Currently, it's working as expected.

* *connector doesn't parse CustomFieldList*. In former versions the connector didn't parse CustomFieldList, so it wasn't included into the SOAP request to _Netsuite_, when the name was different than _customFieldList_. Now it works as expected.
